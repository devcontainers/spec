...
Metadata properties marked with a 🏷 can also be added to an **image label** in addition to devcontainer.json file. The `devcontainer.metadata` image label can contain an array of json snippets that will be automatically merged with devcontainer.json contents (if any) when a container is created.
