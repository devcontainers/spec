...
For each lifecycle hook (eg: `onCreateCommand`) (in [Feature installation order](https://containers.dev/implementors/features/#installation-order)), each command contributed by a Feature is executed in sequence (blocking the next command from executing).   If the user `devcontainer.json` provides a lifecycle command, commands provided by Features are always exexuted _before_ any user-provided lifecycle command.
