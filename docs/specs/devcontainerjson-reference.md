...
Metadata properties marked with a 🏷 in this reference may also be stored in a `devcontainer.metadata` image label so they are automatically picked up with the image is referenced.
